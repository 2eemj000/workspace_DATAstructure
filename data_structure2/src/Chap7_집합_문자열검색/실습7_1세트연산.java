...
class Sets {
	...
	public void SimpleUnion(int i, int j){ // 합집합
	// Replace the disjoint sets with roots i and j, i != j with their union
		int ri = SimpleFind(i); // i의 루트를 찾음 -> ri에 저장
    	int rj = SimpleFind(j); // j의 루트를 찾음 -> rj에 저장
		// i,j는 임의 노드로서 각 집합 i의 root를 찾아 j의 root에 연결 
    	if (ri != rj) { // 같으면 굳이 안합쳐도되니까
            parent[ri] = rj; // i의 루트를 j의 루트에 연결 (집합 병합)
        }
	}
	...
	{
		while (parent[i] > 0) // 음수값은 루트를 의미, parent 배열을 사용하여 각 원소의 부모를 저장
			i = parent[i];
		return i;
	}
